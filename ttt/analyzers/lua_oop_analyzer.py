...
_CID_PARAM_RE = re.compile(r"\bfunction\b[^(]*\([^)]*\bcid\b[^)]*\)")
...
_STORAGE_GLOBAL_RE = re.compile(r"^([a-zA-Z_]\w*)\s*=\s*(\d+)\s*$")
...
class LuaOopAnalyzer:
    def analyze_content(self, content: str, rel_path: str) -> LuaFileAnalysis:
        ...
        return LuaFileAnalysis(
            file_path=rel_path, issues=issues, total_lines=len(lines)
        )
    ...
    def _detect_storage_tables(self, content: str) -> List[LuaOopIssue]:
        ...
        for m in table_block_re.finditer(content):
            ...
            num_vals = [
                int(v)
                for v in re.findall(r"=\s*(\d+)", body)
    ...
    def _detect_global_state(self, lines: list) -> List[LuaOopIssue]:
        ...
        return [
            LuaOopIssue(
                ...
                description=(f"{len(found)} global variable(s) detected: {sample}."),
                guideline=(
