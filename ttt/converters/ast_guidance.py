...
def analyze_converted_code(code: str, original: str, filename: str,
                           report: GuidanceReport) -> None:
...
def _check_deprecated_patterns(code: str, filename: str,
                                report: GuidanceReport) -> None:
    """Detect deprecated TFS 1.x patterns that should be updated."""
    deprecations = [
        (r'\bgetConfigValue\b', "getConfigValue",
         "Use configManager[key] instead of getConfigValue()"),
        (r'\bdoRemoveCreature\b', "doRemoveCreature",
         "Use creature:remove() instead"),
        (r'\bdoRemoveItem\b', "doRemoveItem",
         "Use item:remove() instead"),
        (r'\bdoPlayerSendTextMessage\b', "doPlayerSendTextMessage",
         "Use player:sendTextMessage() instead"),
        (r'\baddEvent\b.*\baddEvent\b', "Nested addEvent",
...
def _check_object_lifecycle(code: str, filename: str,
                             report: GuidanceReport) -> None:
...
def _check_type_coercion(code: str, filename: str,
                          report: GuidanceReport) -> None:
...
def _check_event_registration(code: str, filename: str,
                                report: GuidanceReport) -> None:
...
def _check_storage_usage(code: str, original: str, filename: str,
                          report: GuidanceReport) -> None:
