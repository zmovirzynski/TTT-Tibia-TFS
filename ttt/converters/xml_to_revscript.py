...
class XmlToRevScriptConverter:
    ...
    def _apply_lua_transform(
        self, lua_code: str, script_name: str, fr: "FileReport"
    ) -> str:
    ...
    def convert_xml_file(
        self, xml_path: str, scripts_dir: str, output_dir: str
    ) -> List[str]:
        ...
        if "talkactions" in filename or "talkaction" in filename:
            return self._convert_talkactions(
                xml_content, scripts_dir, output_dir, xml_path
            )
        elif "creaturescripts" in filename or "creaturescript" in filename:
            return self._convert_creaturescripts(
                xml_content, scripts_dir, output_dir, xml_path
            )
        elif "globalevents" in filename or "globalevent" in filename:
            return self._convert_globalevents(
                xml_content, scripts_dir, output_dir, xml_path
            )
            ...
            return self._convert_movements(
                xml_content, scripts_dir, output_dir, xml_path
            )
        else:
    ...
    def _convert_actions(
        self, xml_content: str, scripts_dir: str, output_dir: str, xml_path: str
    ) -> List[str]:
        output_files = []
    ...
    def _generate_combined_action_revscript(
        self, var_name: str, entries: List[Dict], full_code: str, transformed_code: str
    ) -> str:
        ...
        lines.append("")
        lines.append(
            f"function {var_name}.onUse(player, item, fromPosition, target, toPosition, isHotkey)"
        )
        ...
        for entry in entries:
            reg_lines = self._generate_registration_lines(
                var_name, entry, ACTION_REGISTRATION
            )
    ...
    def _generate_action_revscript(
        self, var_name: str, entry: Dict, func_body: str, full_code: str
    ) -> str:
        lines = []
    ...
    def _generate_movement_revscript(
        self,
        var_name: str,
        entry: Dict,
        func_body: str,
        ...
        reg_lines = self._generate_registration_lines(
            var_name, entry, MOVEMENT_REGISTRATION
        )
        lines.extend(reg_lines)
        lines.append(f"{var_name}:register()")
    ...
    def _generate_combined_movement_revscript(
        self, var_name: str, entries: List[Dict], full_code: str, transformed_code: str
    ) -> str:
        ...
        if has_step:
            ...
            lines.append(
                f"function {var_name}.onStepIn(creature, item, position, fromPosition)"
            )
            lines.append(self._indent_code(func_body))
            lines.append("end")
            lines.append("")
            ...
            lines.append(
                f"function {var_name}.onStepOut(creature, item, position, fromPosition)"
            )
            lines.append(self._indent_code(func_body))
            lines.append("end")
            lines.append("")
    ...
    def _generate_combined_talkaction_revscript(
        self, var_name: str, entries: List[Dict], full_code: str, transformed_code: str
    ) -> str:
    ...
    def _generate_talkaction_revscript(
        self, var_name: str, entry: Dict, func_body: str, full_code: str, words: str
    ) -> str:
        lines = []
    ...
    def _generate_creaturescript_revscript(
        self,
        var_name: str,
        entry: Dict,
        func_body: str,
    ...
    def _generate_combined_creaturescript_revscript(
        self, var_name: str, entries: List[Dict], full_code: str, transformed_code: str
    ) -> str:
        ...
        for entry in entries:
            ...
            event_method = CREATUREEVENT_TYPES.get(
                event_type, f"on{event_type.capitalize()}"
            )
    ...
    def _generate_combined_globalevent_revscript(
        self, var_name: str, entries: List[Dict], full_code: str, transformed_code: str
    ) -> str:
    ...
    def _generate_globalevent_revscript(
        self,
        var_name: str,
        entry: Dict,
        func_body: str,
    ...
    def _parse_xml_entries(self, xml_content: str, tag_name: str) -> List[Dict]:
        ...
        for elem in root.iter(tag_name):
            ...
            if (
                entry.get("event") == "script"
                and "value" in entry
                and "script" not in entry
            ):
    ...
    def _extract_top_level_code(self, code: str) -> str:
        ...
        for line in lines:
            ...
            if in_function > 0:
                ...
                if re.search(
                    r"\b(function|if|for|while|do|repeat)\b", stripped
    ...
    def _generate_registration_lines(
        self, var_name: str, entry: Dict, reg_info: Dict
    ) -> List[str]:
        lines = []
