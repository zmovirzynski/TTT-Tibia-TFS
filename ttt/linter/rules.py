...
class DeprecatedApiRule(LintRule):
    ...
    def __init__(self):
        ...
        self._pattern = re.compile(r"\b(" + "|".join(escaped) + r")\s*\(", re.MULTILINE)
...
class UnusedParameterRule(LintRule):
        ...
        for match in self._func_pattern.finditer(code):
            ...
            for param in params:
                ...
                if not pattern.search(func_body):
                    ...
                    col = (
                        match.start(2)
                        - self._line_start(code, match.start(2))
                        + param_pos
                        + 1
                    )
                    issues.append(
    ...
    def _extract_function_body(self, code: str, start: int) -> Optional[str]:
        ...
        re.compile(r"\b(function|if|for|while|repeat|do)\b")
        re.compile(r"\bend\b")
...
class InvalidCallbackSignatureRule(LintRule):
    ...
    def check(self, code: str, lines: List[str], filename: str = "") -> List[LintIssue]:
        ...
        for match in self._func_pattern.finditer(code):
            ...
            all_valid = (
                [old_sig["params"]] + old_sig.get("alt_params", []) + [expected_new]
            )
...
class GlobalVariableLeakRule(LintRule):
    ...
    _assign_pattern = re.compile(r"^(\s*)(\w+)\s*=\s*(?!.*[=<>!~])", re.MULTILINE)
    ...
    def check(self, code: str, lines: List[str], filename: str = "") -> List[LintIssue]:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if re.match(
                r"^\s*(local|function|return|if|else|elseif|end|for|while|repeat|until|do|break)\b",
                line,
            ):
                continue
...
class HardcodedIdRule(LintRule):
    ...
    _call_pattern = re.compile(r"\b(\w+)\s*\(\s*(?:[^,)]+,\s*)?(\d{3,6})\b")
    ...
    def check(self, code: str, lines: List[str], filename: str = "") -> List[LintIssue]:
        ...
        for i, line in enumerate(lines, 1):
            ...
            for match in self._call_pattern.finditer(line):
                ...
                if not any(
                    func_name.endswith(f) or func_name == f for f in self.ID_FUNCTIONS
                ):
                    continue
...
class EmptyCallbackRule(LintRule):
    ...
    def check(self, code: str, lines: List[str], filename: str = "") -> List[LintIssue]:
        ...
        for match in self._func_pattern.finditer(code):
            ...
            if not body_no_comments or body_no_comments in (
                "return true",
                "return false",
            ):
                func_line = code[: match.start()].count("\n") + 1
